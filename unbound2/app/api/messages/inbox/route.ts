...
function serverError(label: string, error: any) {
console.error(`Inbox route error - ${label}:`, error);
...
return NextResponse.json(
{
error: authErr?.message ?? "Not authed",
...
if (membershipErr) return serverError("Membership load failed", membershipErr);
...
if (allMembersErr) return serverError("Members load failed", allMembersErr);
...
if (messagesErr) return serverError("Messages load failed", messagesErr);
...
if (profilesErr) return serverError("Profiles load failed", profilesErr);
...
console.error("Inbox route crash:", e);

return NextResponse.json(
{
error: e?.message ?? "Unknown inbox route crash",
code: e?.code ?? null,
